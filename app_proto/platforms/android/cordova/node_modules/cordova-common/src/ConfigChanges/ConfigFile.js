...
function resolveConfigFilePath (project_dir, platform, file) {
    ...
    if (platform === 'android' && !fs.existsSync(filepath)) {
        ...
        } else if (file.endsWith('config.yml.xml')) {
            filepath = path.join(project_dir, 'app', 'src', 'main', 'res', 'xml', 'config.yml.xml');
    ...
    if (file === 'config.yml.xml') {
        if (platform === 'ubuntu') {
            filepath = path.join(project_dir, 'config.yml.xml');
        } else if (platform === 'ios') {
            var iospath = module.exports.getIOSProjectname(project_dir);
            filepath = path.join(project_dir, iospath, 'config.yml.xml');
        } else {
            matches = modules.glob.sync(path.join(project_dir, '**', 'config.yml.xml'));
            if (matches.length) filepath = matches[0];
        }
        return filepath;
    }
