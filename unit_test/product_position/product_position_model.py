import datetime
from bdd.db_connection import User, Product, Sensor
...
def get_product_model(param=None):
    product = {
        ...
        "id_ean": "3017620422003",
        "position": "placard sous evier",
        "product_name": "Nutella",
        "product_name_gen": "Nutella_gen",
    }
    if param:
        for key in param:
            if key in product:
...
def product_model_to_sql(product):
    ...
    for key in product:
        ...
        if key is 'product_name':
            new_product_object.product_name = product[key]
        if key is 'product_name_gen':
            new_product_object.product_name_gen = product[key]
...
def get_sensor_model(param=None):
    sensor = {
        ...
        'id_user': 1,
        "type": 1,
        "name": "Placard sous évier",
        "sensorcol": "col",
    }
    if param:
        for key in param:
            if key in sensor:
                sensor[key] = param[key]
    return sensor
